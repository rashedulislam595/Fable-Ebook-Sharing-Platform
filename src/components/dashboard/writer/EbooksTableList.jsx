...
export function EbooksTableList({ initialEbooks }) {
  ...
  const handleDelete = (ebookId) => {
    if (confirm("Are you sure you want to delete this ebook permanently?")) {
      startTransition(async () => {
        // API Call here
        setEbooks((prev) => prev.filter(ebook => (ebook._id?.$oid || ebook._id) !== ebookId));
      });
    }
  };
                ...
                <div className="flex items-center justify-end gap-2 pt-2 border-t border-zinc-200/60">
                  <Button 
                    ...
                    className={`w-8 h-8 rounded-md ${
                      ebookStatus === "published" ? "text-zinc-500 hover:text-red-500 hover:bg-zinc-100" : "text-zinc-500 hover:text-emerald-600 hover:bg-zinc-100"
                    }`}
                      ...
                      <Table.Cell>
                        <div className="flex items-center gap-1">
                          <Button 
                            ...
                            disabled={isPending}
                            onClick={() => handleTogglePublish(ebookId, ebookStatus)}
                            className={`min-w-8 w-8 h-8 rounded-md transition-colors ${
                          ...
                          <Button 
                            ...
                            disabled={isPending}
                            onClick={() => handleDelete(ebookId)}
                            className="text-zinc-500 hover:text-red-500 hover:bg-zinc-100 min-w-8 w-8 h-8 rounded-md transition-colors disabled:opacity-30"
                            title="Delete Ebook"
                          >
                            <TrashBin width={14} height={14} />
                          </Button>
